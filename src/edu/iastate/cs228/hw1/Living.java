...
public abstract class Living 
{
	...
	{		
		...
		for(int i = Math.max(0, this.row-1);i<=Math.min(this.row+1, plain.getWidth()-1);i++)
		{
			for(int j = Math.max(0, this.column-1);j<=Math.min(this.column+1, plain.getWidth()-1);j++)
			{
